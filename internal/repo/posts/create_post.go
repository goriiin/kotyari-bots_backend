...
func (p *PostsRepo) CreatePost(ctx context.Context, post model.Post, categoryIDs []uuid.UUID) (model.Post, error) {
...
func (p *PostsRepo) insertPostCategoriesBatch(ctx context.Context, tx pgx.Tx, postID uint64, categoryIDs []uuid.UUID) error {
	if len(categoryIDs) == 0 {
		return nil
	}
