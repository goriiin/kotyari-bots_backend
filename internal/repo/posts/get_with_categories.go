...
func (p *PostsRepo) GetWithCategories(ctx context.Context, id uint64) (model.PostWithCategories, error) {
	...
	dtoCategories, err := pgx.CollectRows(rows, pgx.RowToStructByName[CategoryDTO])
	if err != nil {
	...
	return model.PostWithCategories{
		Post:       post,
		Categories: categoriesDtoToModel(dtoCategories),
	}, nil
}
